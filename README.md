...
### Multiple boards (one dashboard tab each)

Nothing special needed - this is just normal Lovelace behavior, since each
board is a single, independent card. Give the dashboard one **view** (tab)
per board, each with its own `project` (or `projects`) filter, its own
`card_id`, and `panel: true` so that board gets the full tab width to
itself - exactly like the single-board setup above, just repeated once
per project:
...
others - and since it's the only card on its tab, `panel: true` gives it
the full available width (a **Panel** view only ever supports a single
card - HA shows a warning if you configure more than one there).

If you'd rather have several boards stacked/side by side on the *same*
tab instead of separate tabs, that also works - just put multiple
`custom:jira-board-card` entries under one view's `cards:` instead of
spreading them across views, and drop `panel: true` from that view (only
supports one card). On the default **Masonry** view type the cards then
lay out side by side if the screen is wide enough, wrapping to a new row
otherwise; each board's own `.board { overflow-x: auto }` keeps a
4-column board usable even if squeezed into a narrower tile.
